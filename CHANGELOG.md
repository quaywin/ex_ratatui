...
- **Table: `:footer`, `:header_style`, `:footer_style`, `:column_highlight_style`, `:cell_highlight_style`, `:highlight_spacing`.** Footer renders at the bottom of the area; header / footer styles override the table's `:style` for those rows. `column_highlight_style` / `cell_highlight_style` paint the selected column / cell (intersection of selected row + column) — useful for spreadsheet-style navigation. `highlight_spacing` (`:always`, `:when_selected` default — matches ratatui, `:never`) controls when the symbol column is reserved; `:always` pins column positions even when nothing is selected so the row doesn't shift on select.
