...
- **BarChart widget** ([#23](https://github.com/mcass19/ex_ratatui/issues/23)) — new `ExRatatui.Widgets.BarChart` and `ExRatatui.Widgets.Bar` structs render ratatui's `BarChart` in either `:vertical` or `:horizontal` orientation. Each `%Bar{}` carries a `:label`, non-negative integer `:value`, optional per-bar `:style` that overrides the chart-wide `:bar_style`, and an optional `:text_value` to replace the numeric readout. Chart-level fields include `:bar_width`, `:bar_gap`, `:bar_style`, `:value_style`, `:label_style`, `:max` (nil auto-scales to the largest value), `:direction`, and `:block`. Floats and negative values raise `ArgumentError` at the bridge boundary. See the [BarChart section](guides/building_uis.md#barchart) in Building UIs and the widget cheatsheet for examples
