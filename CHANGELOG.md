...
- **Single-binary distribution via [Burrito](https://github.com/burrito-elixir/burrito).** The new [Packaging with Burrito guide](guides/packaging_with_burrito.md) walks through wrapping any `ExRatatui.App` into a self-contained native binary per OS/arch — Linux x86_64, macOS x86_64, macOS aarch64, and Windows x86_64. End users download one file, run it, and the BEAM + ex_ratatui + Rust NIF unpack into a per-user cache on first launch. `examples/burrito_demo/` is the reference consumer, and `.github/workflows/burrito_demo.yml` is the regression CI proving all four targets build + smoke-test green on every push.
...
- **Precompiled `*-unknown-linux-musl` NIFs are now self-contained.** Added `-C link-arg=-static-libgcc` to the musl rustflags in `native/ex_ratatui/.cargo/config.toml`. Previously the published musl artifact dynamically linked `libgcc_s.so.1`, which failed to load under [Burrito](https://github.com/burrito-elixir/burrito)'s musl-based Linux payload. Affects both `x86_64-unknown-linux-musl` and `aarch64-unknown-linux-musl` targets.
