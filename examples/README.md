...
| [`layout_flex_demo.exs`](layout_flex_demo.exs) | `mix run examples/layout_flex_demo.exs` | Reference card for `Layout.split/4` opts. Side-by-side strips demo every `:flex` mode (`:start`, `:center`, `:end`, `:space_between`, `:space_around`), three `{:fill, n}` weighted panels with a `spacing: 2` gutter, and printable rect coordinates so the visual matches the layout. |
