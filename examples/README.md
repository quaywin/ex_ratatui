...
| [`theme_picker.exs`](theme_picker.exs) | `mix run examples/theme_picker.exs` | Visual reference for `ExRatatui.Theme`. Left half: one row per slot showing the slot name and a swatch painted with the slot color. Right half: a live preview rendering the focused (`border_style(focused: true)`) and unfocused (`border_style(focused: false)`) variants plus `text_style/2` (default and `dim:`) and `selection_style/1` in real widgets. Press `1`/`2`/`3` to switch between `default/0`, `light/0`, and a custom Nord-ish theme. |
